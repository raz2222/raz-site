...
async function fetchSupabaseUrls() {
  ...
  for (const g of guides ?? []) {
    ...
    if (isTutorial) hasTutorials = true
    urls.push({
  ...
  // prerenderRoutes only emits /tutorials once a tutorial exists, so listing it
  // unconditionally pointed the sitemap at a route with no prerendered HTML:
  // Google would fetch an empty shell and file it as thin content. The index
  // enters the sitemap on the day it has something to index.
  if (hasTutorials) urls.push({ loc: `${BASE}/tutorials`, changefreq: "weekly", priority: 0.6 })
