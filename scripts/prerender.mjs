...
async function fetchSsrData() {
  ...
  const [guides, subServices, serviceHubs, faqGroups, projects, siteContent] = await Promise.all([
    ...
    // The SEO written in /admin/pages. Without this the admin's title fields
    // changed the browser tab and nothing a crawler ever read, which is the
    // opposite of what a field labelled "כותרת לגוגל" promises.
    supabase.from("site_content").select("key, value").like("key", "seo_%"),
  ])

  for (const result of [guides, subServices, serviceHubs, faqGroups, projects, siteContent]) {
  ...
  return {
    ...
    pageSeo: Object.fromEntries((siteContent.data ?? []).map((row) => [row.key, row.value ?? {}])),
  }
}
 ...
 *
 * `meta` is resolveRouteMeta("/"), so a title written in /admin/pages reaches
 * the one page whose <head> is index.html's own rather than a generated
 * snapshot. With nothing written there it holds index.html's existing strings
 * and patching is a no-op. */
export async function prerenderHomepage(render, data, template, meta) {
  ...
  const head = meta ? patchHead(template, meta) : template
  await writeFile(distIndexPath, head.replace(ROOT_PLACEHOLDER, `<div id="root">${appHtml}</div>`), "utf-8")

  return { appHtml, chars: text.length, meta }
}
...
async function main() {
  ...
  try {
    ...
    const { render, renderPage, listPrerenderRoutes, resolveRouteMeta } = await import(pathToFileURL(entryPath).href)
    ...
    const homeMeta = resolveRouteMeta("/", data)
    const { appHtml, chars } = await prerenderHomepage(render, data, template, homeMeta)
    console.log(`Prerendered "/" into dist/index.html (${chars} chars of visible text).`)

    const markdown = htmlToMarkdown(appHtml, {
      title: homeMeta?.title,
      description: homeMeta?.description,
      canonical: homeMeta?.canonical ?? "https://madebyraz.co.il/",
    })
