...
function Block({ n, label, text }: { n: string; label: string; text: string }) {
  return (
    <Reveal className="grid md:grid-cols-[120px_1fr] gap-4 md:gap-14 border-t border-white/10 pt-10 md:pt-12">
      ...
      <p className="text-2xl md:text-3xl leading-[1.35] max-w-2xl font-display font-light">{text}</p>
    </Reveal>
  )
}
      ...
      <section className="pt-28 pb-8 md:pt-36">
        <div className="container">
          <Reveal className="font-mono text-xs uppercase tracking-wide text-dim mb-4 flex items-center gap-3">
            ...
            {project.concept && (
              <span className="border border-white/20 rounded-full px-3 py-0.5">פרויקט קונספט עצמאי</span>
            )}
          </Reveal>
          <Reveal>
          ...
          {project.tools?.length > 0 && (
            <Reveal className="grid md:grid-cols-[120px_1fr] gap-4 md:gap-14 border-t border-white/10 pt-10 md:pt-12">
              ...
              <div className="flex flex-wrap gap-3">
                {project.tools.map((t) => (
                  <span key={t} className="border border-white/30 rounded-full px-4 py-2 text-sm">
                    {t}
                  </span>
                ))}
              </div>
