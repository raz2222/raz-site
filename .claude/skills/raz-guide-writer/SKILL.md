...
5. **Assign metadata:**
   ...
   - `read_time`: compute it from the actual body text instead of guessing. The site's labels were audited and corrected in Sep 2026 after every guide claimed "6 דקות קריאה" for what was really a 2-minute read. Current calibration across the live guides: roughly 700-1,200 chars of body text is `"2 דקות קריאה"`, ~1,800 chars is `"3 דקות קריאה"`, and 2,300+ chars is `"4 דקות קריאה"`. The English mirror carries the matching `"N min read"`.
